...
public class SemanticResolver implements Expression.Visitor<Void>, Statement.Visitor<Void> {
    ...
    public Void visit(Expression.Variable statement) {
        if (!scopes.isEmpty() &&
            ...
            !scopes.peek().get(statement.name().lexeme()).defined) {
                E30.error(statement.name(), "Cannot read local variable in its own initializer.");
        }
    ...
    public Void visit(Statement.Return statement) {
        if (currentFunction == FunctionType.NONE) {
            E30.error(statement.keyword(), "Cannot return from top-level code.");
        }

        if (statement.value() != null) {
            if (currentFunction == FunctionType.DEFINITION) {
                E30.error(statement.keyword(), "Cannot return a value from a definition.");
            }
            resolve(statement.value());
        }
    ...
    public Void visit(Statement.Node statement) {
        ...
        if (parent != null) {
            if (statement.name().lexeme().equals(parent.name().lexeme())) {
                E30.error(parent.name(), "A node cannot inherit from itself.");
            }
    ...
    public Void visit(Self expression) {
        if (currentNodeType == NodeType.NONE) {
            E30.error(expression.keyword(), "Cannot use 'self' outside of a node.");
            return null;
        }
    ...
    public Void visit(Parent expression) {
        if (currentNodeType == NodeType.NONE) {
            E30.error(expression.keyword(), "Cannot use 'parent' outside of a node.");
            return null;
        }
        if (currentNodeType == NodeType.TRAIT) {
            E30.error(expression.keyword(), "Cannot use 'parent' in a trait.");
            return null;
        }
        if (currentNodeType != NodeType.CHILD) {
            E30.error(expression.keyword(), "Cannot use 'parent' in a node with no parent.");
            return null;
        }
    ...
    private void declare(Token name) {
        ...
        if (scope.containsKey(name.lexeme())) {
            E30.error(name, "Variable with this name already declared in this scope.");
        }
